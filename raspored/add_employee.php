...
if ($name === '' || $last === '' || $birth === '') {
    header("Location: " . BASE_URL . "/index.php?year=$year&month=$month&error=" . urlencode("All fields are required."));
    exit;
}
...
");
if (!$stmt) {
    header("Location: " . BASE_URL . "/index.php?year=$year&month=$month&error=" . urlencode("Database error (prepare)."));
    exit;
}
...
if (!$stmt->execute()) {
    header("Location: " . BASE_URL . "/index.php?year=$year&month=$month&error=" . urlencode("Employee already exists (first name + last name + date of birth)."));
    exit;
}
